...
int main() {
    ...
    MyProperty* result = myproperty_plus(&obj->speed->base, 2 * obj->accel->base.value);
    myobject_set_speed(obj, (Speed*)result);

    myobject_print(obj, &printer);
    
    printer_printline(&printer, "\nAfter 3s deceleration:");

    result = myproperty_minus(&obj->speed->base, 3 * obj->accel->base.value);
    myobject_set_speed(obj, (Speed*)result);
